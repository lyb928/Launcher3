...
public class AppsCustomizeTabHost extends TabHost implements LauncherTransitionable,
        TabHost.OnTabChangeListener, BackgroundAlphable  {
    ...
    private float mBackgroudAlpha = 0.25f;
...
//                mAppsCustomizePane.hideScrollingIndicator(false);
	...
	protected void onDraw(Canvas canvas) {
		if (mBackgroudAlpha > 0.0f) {
			canvas.drawARGB((int) (mBackgroudAlpha * 255), 0, 0, 0);
		}
		super.onDraw(canvas);
	}
...
//		super.setBackgroundColor(color);
		mBackgroudAlpha = (float)(color >> 24) / 255f;
		invalidate();
	}

	/**
	 * We dispatch touchevent to AppCustomize directly in previews case to prevents touchevent handled by TabWidget,
	 * which should be invisible when previews happens.  added by leeyb
	 */
    @Override
    public boolean dispatchTouchEvent(MotionEvent event) {
        if (mAppsCustomizePane.isPreviewsState()) {
            final float offsetX = mScrollX - mAppsCustomizePane.getLeft();
            final float offsetY = mScrollY - mAppsCustomizePane.getTop() - mTabsContainer.getMeasuredHeight();
            event.offsetLocation(offsetX, offsetY);
            return mAppsCustomizePane.dispatchTouchEvent(event);
        }
