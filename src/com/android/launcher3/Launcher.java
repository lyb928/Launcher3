...
//    private View mQsbDivider;
    private SlideDivider mDockDivider;
...
//        mQsbDivider = (ImageView) findViewById(R.id.qsb_divider);
        mDockDivider = (SlideDivider) findViewById(R.id.dock_divider);
        dragController.addDropTarget(mWorkspace);
        //modified by leeyb, the workspace in DragTarget list must be the first position
        // Setup the hotseat
        mHotseat = (Hotseat) findViewById(R.id.hotseat);
        if (mHotseat != null) {
            mHotseat.setup(this, mDragController);
        }

        // Setup the workspace
//        mWorkspace.setHapticFeedbackEnabled(false);
        syncWorkspacePages();
        syncWorkspaceDefaultPage();
        mWorkspace.setOnLongClickListener(this);
        mWorkspace.setup(dragController, mDockDivider, mHotseat);
        ...
        mTab = mAppsCustomizeTabHost.findViewById(R.id.tabs_container);
        SlideDivider dividerApp = (SlideDivider) mAppsCustomizeTabHost.findViewById(R.id.dock_divider);
        mAppsCustomizeContent.setup(this, dragController, dividerApp);
...
//            }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (Intent.ACTION_MAIN.equals(intent.getAction())) {
            ...
            if (isAllAppsVisible() && mAppsCustomizeContent.isPreviewsState()) {
				mAppsCustomizeContent.showPreviews(false);
			}
...
//        }
    ...
    public void onBackPressed() {
        ...
        if (mState == State.APPS_CUSTOMIZE || mWorkspace.isPreviewsState()) {
        	if (isAllAppsCustomizeOpen() && mAppsCustomizeContent.isPreviewsState()) {
        		mAppsCustomizeContent.showPreviews(false);
			} else {
				showWorkspace(true);
			}
     ...
     * of the screen. modified by leeyb
     */
    private void showAppsCustomizeHelper(boolean animated, final boolean springLoaded) {
        ...
        final View translateView = mAppsCustomizeTabHost.getTabContentView();
...
//        setPivotsForZoom(toView, scale);
        ...
        if (animated) {
        	final float heightApp = translateView.getMeasuredHeight() * 0.3f;
            final ValueAnimator translateAnim = ValueAnimator.ofFloat(0f, 1f).setDuration(duration);
//            translateAnim.setInterpolator(new Workspace.ZoomOutInterpolator());
            translateAnim.addUpdateListener(new LauncherAnimatorUpdateListener() {
                public void onAnimationUpdate(float a, float b) {
//                    toView.setScaleX(a * scale + b * 1f);
//                    toView.setScaleY(a * scale + b * 1f);
                	translateView.setScaleY(0.8f + 0.2f * b);
                	translateView.setScaleX(0.85f + 0.15f * b);
                	translateView.setTranslationY(heightApp * a);
//                	mWorkspace.setRotationX(45f * a);
//                	toView.setRotationX(- 45f * a);
//                	mWorkspace.setTranslationY(heightWork * a);
                }
            });
//            toView.setAlpha(0f);
...
//                	toView.setAlpha(/*a * 0f +*/ b/* * 1f*/);
                	toView.setBackgroundColor(Color.argb((int) (100f * b), 0, 0, 0));
                	translateView.setAlpha(b);
//                    mWorkspace.setAlpha(a);
                }
            });
            mTab.animate().alpha(1f).setDuration(duration / 2).start();
...
//                    toView.setTranslationX(0.0f);
                	toView.setBackgroundColor(Color.argb(0, 0, 0, 0));
...
//                	mHotseat.setVisibility(View.INVISIBLE);
                	translateView.setScaleX(1.0f);
                	translateView.setScaleY(1.0f);
                	toView.setAlpha(1f);
...
//                        updateWallpaperVisibility(false);
            ...
            mStateAnimation.setStartDelay(60l);
            mStateAnimation.playTogether(translateAnim, alphaAnim)/*.after(startDelay)*/;
...
//        	mHotseat.setVisibility(View.INVISIBLE);
        	toView.setBackgroundColor(Color.argb(100, 0, 0, 0));
            toView.setTranslationX(0.0f);
            toView.setTranslationY(0.0f);
            toView.setScaleX(1.0f);
            toView.setScaleY(1.0f);
...
//            updateWallpaperVisibility(false);
     ...
     * @param animated If true, the transition will be animated.	modified by leeyb
     */
...
//        showHotseat(animated);
        if (animated) {
        	final float height = translateView.getMeasuredHeight() * 0.3f;
            final float oldScaleX = fromView.getScaleX();
            final float oldScaleY = fromView.getScaleY();
            final float oldDividerY = mDockDivider.getTranslationY();
            final float oldSeatY = mHotseat.getTranslationY();

            ValueAnimator translateAnim = ValueAnimator.ofFloat(0f, 1f).setDuration(duration);
//            translateAnim.setInterpolator(new Workspace.ZoomInInterpolator());
            translateAnim.addUpdateListener(new LauncherAnimatorUpdateListener() {
                public void onAnimationUpdate(float a, float b) {
//                    fromView.setScaleX(a * oldScaleX + b * scaleFactor);
//                    fromView.setScaleY(a * oldScaleY + b * scaleFactor);
                	translateView.setScaleY(0.8f + 0.2f * a);
                	translateView.setScaleX(0.85f + 0.15f * a);
                	translateView.setTranslationY(height * b);
                }
            });
            ...
            alphaAnim.addUpdateListener(new LauncherAnimatorUpdateListener() {
                public void onAnimationUpdate(float a, float b) {
                	translateView.setAlpha(a /** 1f + b * 0f*/);
                	fromView.setBackgroundColor(Color.argb((int) (100f * a), 0, 0, 0));
//                    fromView.setRotationX(45f * b);
                }
            });
            mTab.animate().setDuration(duration / 2).alpha(0.0f).start();
...
//                    updateWallpaperVisibility(true);
                    ...
                    translateView.setScaleX(1.0f);
                    translateView.setScaleY(1.0f);
                    fromView.setBackgroundColor(Color.argb(0, 0, 0, 0));
                    if (fromView instanceof LauncherTransitionable) {
...
//            mHotseat.setVisibility(View.VISIBLE);
            mStateAnimation = new AnimatorSet();
            mStateAnimation.playTogether(translateAnim, alphaAnim);
            mStateAnimation.start();
        } else {
//        	mHotseat.setVisibility(View.VISIBLE);
        	fromView.setBackgroundColor(Color.argb(0, 0, 0, 0));
...
//            showDockDivider(animated && mState == State.APPS_CUSTOMIZE_SPRING_LOADED);
    ...
    void showAllApps(final boolean animated) {
        if (LauncherLog.DEBUG) {
...
//                mQsbDivider.setAlpha(1f);
            ...
            if (animated) {
                ...
                mDividerAnimator.setDuration(/*mSearchDropTargetBar.getTransitionInDuration()*/150);
...
//                int duration = mSearchDropTargetBar.getTransitionInDuration();
                mHotseat.animate().alpha(1f).setDuration(/*duration*/150);
            } else {
...
//                int duration = mSearchDropTargetBar.getTransitionOutDuration();
                mHotseat.animate().alpha(0f).setDuration(/*duration*/150);
            } else {
            ...
            switch (item.itemType) {
                ...
                case LauncherSettings.Favorites.ITEM_TYPE_FOLDER:
                    FolderIcon newFolder = FolderIcon.fromXml(R.layout.folder_icon, this,
                            (ViewGroup) workspace.getPageAt(workspace.getCurrentPage()),
...
//        mSearchDropTargetBar.onSearchPackagesChanged(searchVisible, voiceVisible);
    ...
    /* Cling related */
    private static final String PREFS_KEY = "com.android.launcher3.prefs";
    ...
    void rememberDesktopOrder() {
        SharedPreferences pref = getSharedPreferences(PREFS_KEY, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = pref.edit();
        final int count = mWorkspace.getPageCount();
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < count; i++) {
            View child = mWorkspace.getPageAt(i);
            sb.append(String.valueOf(mWorkspace.indexOfChild(child)));
        }
        editor.putString(Workspace.WORKSPACE_CELLLAYOUT_ORDER, sb.toString());
        Log.v("leeyb", "rememberDesktopOrder:" + sb.toString());
        editor.commit();
    }
    
    private void syncWorkspacePages() {
        SharedPreferences pref = getSharedPreferences(PREFS_KEY, Context.MODE_PRIVATE);
        String args = pref.getString(Workspace.WORKSPACE_CELLLAYOUT_ORDER, Workspace.DEFAULT_WORKSPACE_ORDER_KEY);
        int pageCount = args.length();
        for (int i = 0; i < pageCount; i++) {
            generateCellLayout(true);
        }
        mWorkspace.restorePageOrderFromKey(args);
    }

    CellLayout generateCellLayout(boolean attachToWorkspace) {
        LayoutInflater inflater = getLayoutInflater();
        if (attachToWorkspace) {
            Workspace local = (Workspace) inflater.inflate(R.layout.workspace_screen, mWorkspace, attachToWorkspace);
            return (CellLayout) local.getChildAt(local.getChildCount() - 1);
        }
        return (CellLayout) inflater.inflate(R.layout.workspace_screen, mWorkspace, attachToWorkspace);
    }

    private void syncWorkspaceDefaultPage() {
        SharedPreferences pref = getSharedPreferences(PREFS_KEY, Context.MODE_PRIVATE);
        int home = pref.getInt(Workspace.WORKSPACE_DEFAULT_PAGE, 3);
        mWorkspace.restoreHomePageFromKey(home);
    }
    
    void rememberDefaultHomePage(int defaultPage) {
        SharedPreferences pref = getSharedPreferences(PREFS_KEY, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = pref.edit();
        editor.putInt(Workspace.WORKSPACE_DEFAULT_PAGE, defaultPage);
        editor.commit();
    }
    
    public void stepToNext(View v) {
        Cling cling = (Cling) findViewById(R.id.workspace_cling);
        ViewGroup first = (ViewGroup) cling.findViewById(R.id.workspace_cling1);
        ViewGroup second = (ViewGroup) cling.findViewById(R.id.workspace_cling2);
        jumpToNext(first, second);
    }

    public void stepToLast(View v) {
        Cling cling = (Cling) findViewById(R.id.workspace_cling);
        ViewGroup first = (ViewGroup) cling.findViewById(R.id.workspace_cling2);
        ViewGroup second = (ViewGroup) cling.findViewById(R.id.workspace_cling3);
        jumpToNext(first, second);
    }

    private void jumpToNext(final ViewGroup first, final ViewGroup second) {
        first.animate().alpha(0.0f).setDuration(DISMISS_CLING_DURATION).setListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
            ...
            public void onAnimationStart(Animator animation) {
                ...
                second.animate().alpha(1.0f).setDuration(DISMISS_CLING_DURATION);
            }
        });
    }
}
