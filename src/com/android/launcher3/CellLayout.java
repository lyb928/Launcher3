...
public class CellLayout extends ViewGroup implements BackgroundAlphable {
	...
	private Drawable mNormalShadow;
	private Drawable mHomeHint;
	private Drawable mDefaultHomeHint;

	private int mShadowOffset;
    private int mRawWidth;
    private int mRawHeight;

    private boolean mBackgroundFocused;
    private PorterDuffColorFilter mBgFocused;
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mNormalBackground = res.getDrawable(R.drawable.panel_frame);
        mNormalShadow = res.getDrawable(R.drawable.panel_highlight);
        mActiveGlowBackground = res.getDrawable(R.drawable.homescreen_blue_strong_holo);
        mHomeHint = res.getDrawable(R.drawable.model_set_btn_normal);
        mDefaultHomeHint = res.getDrawable(R.drawable.model_set_btn_select);
        ...
        mShadowOffset = res.getDimensionPixelSize(R.dimen.shadow_top_offset);
        mBgFocused = new PorterDuffColorFilter(res.getColor(R.color.delete_target_hover_tint),
...
//                        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // backgrounds
        if (mBackgroundAlpha > 0.0f) {//replaced by drawing panelframe and panelshadow
            Drawable bg = mNormalBackground;
            Drawable shadow = mNormalShadow;
...
//            }
//
            View look = (View) getParent();
            boolean isPreviews = false;
            boolean beenPreviews = false;
            if (look instanceof Workspace) {
                Workspace workspace = (Workspace) look;
                isPreviews = workspace.isPreviewsState();
                beenPreviews = workspace.haveBeenPreviews();
            }
            boolean saved = false;
            float offsetW = (float)mRawWidth / getMeasuredWidth();
            float offsetH = (float)mRawHeight / getMeasuredHeight();
            if (bg != null) {
            	bg.setAlpha((int) (mBackgroundAlpha/* * mBackgroundAlphaMultiplier*/ * 255));
                bg.setBounds(mBackgroundRect);
                canvas.save();
                canvas.scale(isPreviews ? offsetW : 1.06f, isPreviews ? offsetH : 1.0f,
                        mScrollX + getWidth() * 0.5f, mScrollY + getHeight() * 0.5f);
                if (isPreviews && mBackgroundFocused) {
                    bg.setColorFilter(mBgFocused);
                } else {
                    bg.clearColorFilter();
                }
                bg.draw(canvas);
                if (beenPreviews) {
                    Drawable hint = mHome ? mDefaultHomeHint : mHomeHint;
                    canvas.save();
                    canvas.translate(getMeasuredWidth() - hint.getIntrinsicWidth() * 2, 0);
                    hint.draw(canvas);
                    canvas.restore();
                }
                canvas.restore();
            }
            if (shadow != null && !isPreviews) {
            	shadow.setAlpha((int) (mBackgroundAlpha/* * mBackgroundAlphaMultiplier*/ * 255));
                shadow.setBounds(mBackgroundRect);
                canvas.save();
                canvas.clipRect(mBackgroundRect.width() * (1 - offsetW), mBackgroundRect.top,
                        mBackgroundRect.width() * offsetW, mBackgroundRect.bottom);
    	        canvas.translate(getTranslationX(), - mShadowOffset);
                shadow.draw(canvas);
                canvas.restore();
			}
        }
        ...
        if (mCrosshairsVisibility > 0.0f) {
            ...
            int x = getPaddingLeft() - (mWidthGap / 2) - (width / 2) + mWidthGap + mCellWidth;
            for (int col = 1; col < countX; col++) {
                int y = getPaddingTop() - (mHeightGap / 2) - (height / 2) + mHeightGap + mCellHeight;
                for (int row = 1; row < countY; row++) {
        ...
        for (int i = 0; i < mFolderOuterRings.size(); i++) {
            ...
            // Draw outer ring --> replaced by drawing folder
            ...
            canvas.save();
            if (fra.mIsCreate) {
                float alpha = fra.getAlpha();
                width = (int) (FolderRingAnimator.sPreviewSize * (1 + FolderIcon.OUTER_RING_GROWTH_FACTOR));
            ...
            // Draw inner ring --> no inner ring anymore, so delete it
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (mInterceptTouchListener != null && mInterceptTouchListener.onTouch(this, ev)) {
        	Log.v("TouchDetector", "onInterceptTouchEvent->CellLayout event:" + ev.getAction() + "\thandle:" + true);
            return true;
        }
        Log.v("TouchDetector", "onInterceptTouchEvent->CellLayout event:" + ev.getAction() + "\thandle:" + false);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int usedInMeasureWidthGap = -1;
        int usedInMeasureHeightGap = -1;
...
//            mHeightGap = Math.min(mMaxGap,numHeightGaps > 0 ? (vFreeSpace / numHeightGaps) : 0);
            ...
            mWidthGap = Math.min(mMaxGap, numWidthGaps > 0 ? (hFreeSpace / numWidthGaps) : 0);
            mHeightGap = Math.min(mMaxGap,numHeightGaps > 0 ? (vFreeSpace / numHeightGaps) : 0);
            usedInMeasureWidthGap = Math.min(mMaxGap, numWidthGaps > 0 ? ((hSpace - mCountX * mOriginalCellWidth) / numWidthGaps) : 0);
            usedInMeasureHeightGap = Math.min(mMaxGap,numHeightGaps > 0 ? ((vSpace - mCountY * mOriginalCellHeight) / numHeightGaps) : 0);
            mChildren.setCellDimensions(mCellWidth, mCellHeight, mWidthGap, mHeightGap);
        } else {
            usedInMeasureWidthGap = mWidthGap = mOriginalWidthGap;
            usedInMeasureHeightGap = mHeightGap = mOriginalHeightGap;
        }
        ...
        if (widthSpecMode == MeasureSpec.AT_MOST) {
            newWidth = mPaddingLeft + mPaddingRight + (mCountX * mOriginalCellWidth) +
                ((mCountX - 1) * usedInMeasureWidthGap);
            newHeight = mPaddingTop + mPaddingBottom + (mCountY * mOriginalCellHeight) +
                ((mCountY - 1) * usedInMeasureHeightGap);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mHomeHint.setBounds(0, 0, mHomeHint.getIntrinsicWidth() * 2, mHomeHint.getIntrinsicHeight() * 2);
        mDefaultHomeHint.setBounds(0, 0, mDefaultHomeHint.getIntrinsicWidth() * 2, mDefaultHomeHint.getIntrinsicHeight() * 2);
    }
    ...
    protected boolean onSetAlpha(int alpha) {
        return true/*!(getParent() instanceof Hotseat)*/;
    }
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    public void markCellsForView(int cellX, int cellY, int spanX, int spanY, boolean value) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		Log.v("TouchDetector", "onTouchEvent->CellLayout event:" + event.getAction() + "\thandle:" + handle);
		return true;
	}
...
//		super.setBackgroundResource(resid);
    ...
    public void rememberRawSize(int width, int height) {
        mRawWidth = width;
        mRawHeight = height;
    }

    public boolean checkHomeHintClicked(int x, int y) {
        getHitRect(mRect);
        Rect hint = mHome ? mDefaultHomeHint.getBounds() : mHomeHint.getBounds();
        float horizontalNormalize = (getMeasuredWidth() - hint.width() * 1.5f) / (float)getMeasuredWidth();
        float verticalNormalize = hint.height() * 1.5f / (float)getMeasuredHeight();
        boolean judgeHorizontal = (x - mRect.left) / (float)mRect.width() >= horizontalNormalize;
        boolean judgeVertical = (y - mRect.top) / (float)mRect.height() <= verticalNormalize;
        return judgeHorizontal && judgeVertical;
    }
    ...
    public void getHitRect(Rect outRect) {
        ...
        if (getParent() instanceof Workspace) {
            if (((Workspace) getParent()).haveBeenPreviews()) {
                int height = outRect.height();
                int width = outRect.width();
                int desireHeight = (int) (height * (float)mRawHeight / getMeasuredHeight());
                int desireWidth = (int) (width * (float)mRawWidth / getMeasuredWidth());
                int verticalOffset = (desireHeight - height) / 2;
                int horizontalOffset = (desireWidth - width) / 2;
    ...
    public void setBackgroundFocused(boolean show) {
        if (mNormalBackground != null ) {
            mBackgroundFocused = show;
            invalidate();
        }
    }
}
